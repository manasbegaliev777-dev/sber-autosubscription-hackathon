...
CatBoostClassifier(
    ...
    scale_pos_weight=46.7, # компенсация дисбаланса классов 97.9/2.1
    eval_metric='AUC',
    random_seed=42
)
```
...
| Модель | ROC-AUC |
|--------|---------|
| DummyClassifier (random baseline) | 0.4991 |
| Logistic Regression (бейзлайн) | 0.6346 |
| **CatBoost (финальная модель)** | **0.7289** |
| Целевой порог по заданию | ≥ 0.6500 |

Целевой порог превышен на **+0.079**.
...
300:  AUC = 0.7241
400:  AUC = 0.7268
499:  AUC = 0.7289  ← лучший результат
```
...
| Метрика | Значение |
|---------|----------|
| Среднее время ответа | 5.88 мс |
| Медиана | 5.87 мс |
| 99-й перцентиль | 8.14 мс |
| Максимум | 10.69 мс |
...
| ML-модель | 35 | ROC-AUC = **0.7289** (цель ≥ 0.65), утечка данных устранена |
| Интерпретация фичей | 10 | Feature Importance + SHAP (bar + beeswarm) |
| API / скрипт | 15 | FastAPI, 4 эндпоинта, время ответа < 12 мс |
